...
window.AD = window.AD || {};
AD.BUILD = '141';   // bumped every deploy; shown on the title so a stale cache is obvious
...
AD.BOREDOM_MAX = { rookie: 58, standard: 44, historic: 30 };
...
/* The President gets bored just from TIME. Every month adds this much boredom on
   its own, so a term of nothing but sober governing climbs toward the danger
   line and the Phone / the wild options become real maintenance. */
AD.BORED_DRIFT = 1.2;

/* ROOM-USAGE INSTRUMENTATION. The overlay ids that count as management rooms, so
   the engine can tally which rooms a player actually opens across a term (see
   AD.UI.overlay and the end-of-term dossier). */
AD.ROOM_IDS = { senate: 'The Caucus', press: 'The Press Room', street: 'Public Order',
  courts: 'The Bench', basepop: 'The Base', call: 'The Phone', war: 'The War Room',
  economy: 'The Economy', diplomacy: 'The State Department', corruption: 'Private Wealth',
  pardon: 'Clemency', renovations: 'The Residence', constitution: 'The Constitution' };
AD.boredCeiling = run => {
...
/* ---------- per-entity effect jitter (uniqueness) -------------------------
   Some phone contacts and diplomacy summits share identical effect NUMBERS (the
   text is always unique). This nudges the meter keys by a deterministic -1..+1
   derived from who/what you are acting on, so no two entities resolve to the
   same numbers, leaving fun / cash / rel / auth untouched. Never shown in a
   preview, only applied. Same idea as AD.varyBySenator / AD.varyByNation. */
AD.jitterEff = function (eff, seedKey) {
  if (!eff) return eff;
  let h = 2166136261;
  const s = String(seedKey || '');
  for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); }
  let bit = h >>> 0;
  ['base', 'congress', 'courts', 'press', 'street'].forEach(k => {
    if (!eff[k]) return;
    const j = (bit % 3) - 1;
